...
""".replace('█', f'{b}█{y}'))
    print(f"""
{y}[{Fore.GREEN}!{y}]{w} {Fore.GREEN}Yaga SelfBot est maintenant en ligne et prêt !{w}
          
{y}[{Fore.GREEN}!{y}]{w} {Fore.GREEN}Informations du Yaga SelfBot:

Version : v{__version__}
Connecté en tant que : {bot.user} ({bot.user.id})
Utilisateurs en cache : {len(bot.users)}
Serveurs connectés : {len(bot.guilds)}
Préfixe du SelfBot : {prefix}
""")
...
async def on_ready():
    if platform.system() == "Windows":
        ctypes.windll.kernel32.SetConsoleTitleW(f"SelfBot v{__version__} - Made By bendé")
        os.system('cls')
    else:
        os.system('clear')
    selfbot_menu(bot)
...
async def help(ctx):
    ...
    help_text = f"""
**Bendé SelfBot | Prefix: `{prefix}`**\n
**Commands:**\n
> :space_invader: `{prefix}bendé` - Show my social networks.
